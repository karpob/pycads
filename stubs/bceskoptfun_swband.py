#!/usr/bin/env python3
from namelist_io import getParmAllSwMw, getParmLwMw, writeNamelist, getParmSwMw3band, getParmLwMwOpen, getParmAllSwMwOpen
...
def replaceParms(x,d):
    ...
    #d['R__Window_Grad_Threshold'][1] = x[1]  
    #d['N__GradChkInterval'][1] =int(x[2])  
...
if __name__ == "__main__":
    ...
    print(all_cloud_flags_lw_mw.shape)
    for i,c in enumerate(np.unique(inputOdb['chan'][0])):
        print(c,i,np.count_nonzero(all_cloud_flags_lw_mw[:,i]))

    #output_h5('ctl.h5',all_cloud_flags_lw_mw,inputOdb, 27)
    x00 = Real(name='x00',low=0.3, high=1.2)
    x01 = Integer(name='x01',low=3, high=15)
    #x02 = Integer(name='x02',low=4, high=6)
    #x03 = Integer(name='x03',low=1, high=10)
    #x01 = Integer(name='x01',low=1,high=10)
    ...
    def objective( x00,x01 ):
        ...
        writeNamelist('SW.NL',d_init)
        d = replaceParms(xx,d_init)
        writeNamelist('SW.NL_TMP',d)
        writeNamelist('CRIS_CLDDET.NL',d)
        time.sleep(1)
        ...
        #chans_use = np.asarray([1189, 1961, 1962, 1963, 1964, 1965, 1966, 1967, 1968,\
        #           1969, 1970, 1971, 1972, 1973, 1974, 1975, 1976, 1977, 1978,\
        #           1979, 1980, 1981, 1982, 1983, 1984, 1985, 1986, 1987, 2119,\
        #           2140, 2143, 2147, 2153, 2158, 2161, 2168, 2171, 2175, 2182])
        chans_use =np.asarray( [ 1189, 2182])

        #chans_use = np.asarray([1939, 1940, 1941, 1942, 1943, 1944, 1945, 1946, 1947, 1948, 1949, 1950, 1951, 1952,\
        #                 1953, 1954,1955, 1956, 1957, 1958, 1959, 1960, 1961, 1962, 1963, 1964, 1965, 1966,\
        #                 1967, 1968, 1969, 1970, 1971, 1972, 1973, 1974, 1975, 1976, 1977, 1978, 1979, 1980,\
        #                 1981, 1982, 1983, 1984, 1985, 1986, 1987])

        #chans_use = np.asarray([ 1981, 1982, 1983, 1984, 1985, 1986, 1987, 2119, 2140, 2143, 2147, 2153, 2158, 2161,\
        #                 2168, 2171, 2175, 2182])



        omb = np.asarray(inputOdb['ob']) - np.asarray(inputOdb['bkg'])
        consider_idx, = np.nonzero(np.in1d(chans,chans_use))
        print('chan idx used',consider_idx)
        print('chans used.',chans[consider_idx])
        ytrue = flags_in[:,consider_idx]#.flatten().astype('float')
        ypred = new_flags[:,consider_idx]#.flatten().astype('float')
        all_clear_count = np.count_nonzero( (np.all(ytrue<1,axis=1)) & (np.all(ypred<1,axis=1)) ) 
        all_cld_count = np.count_nonzero( (np.all(ytrue>0,axis=1)) & (np.all(ypred>0,axis=1)) ) 
        match_count = 0
        for ii in range(0,ytrue.shape[0]):
            if(np.all(ytrue[ii,:]==ypred[ii,:])):
                match_count+=1
        idx_no_match = np.nonzero(np.all(ytrue!=ypred,axis=1))
        print(idx_no_match) 
        ombs = omb[:,consider_idx] 
        oomb = []
        nomb = []
        skewt = []
        skewp = []
        kurtt = []
        kurtp = [] 
        for ii in range(ombs.shape[1]):
            tidx, = np.where(ytrue[:,ii]<1.0)
            pidx, = np.where(ypred[:,ii]<1.0)
            oomb.append(omb[tidx,ii].mean())    
            nomb.append(omb[pidx,ii].mean())
            skewt.append(scipy.stats.skew(ombs[tidx,ii]))    
            skewp.append(scipy.stats.skew(ombs[pidx,ii]))
            kurtt.append(scipy.stats.kurtosis(ombs[tidx,ii]))
            kurtp.append(scipy.stats.kurtosis(ombs[pidx,ii]))
        print('nomb',nomb) 
        skewp = np.asarray(skewp).mean()    
        oomb = np.asarray(oomb).mean()
        nomb = np.asarray(nomb).mean()
        skewt = np.asarray(skewt).mean()    
        skewp = np.asarray(skewp).mean()   
 
        kurtt = np.asarray(kurtt).mean()-3.0    
        kurtp = np.asarray(kurtp).mean()-3.0    
        oomb = np.asarray(oomb).mean()
        nomb = np.asarray(nomb).mean()
        cost = nomb 
        ...
        #cnt0 = len(idx)
        true_cnt = np.count_nonzero(ytrue>0)
        pred_cnt = np.count_nonzero(ypred>0)
        #idx, = np.where((ytrue>0.0) & (ypred>0.0))
        #cnt1 = len(idx)

        #idx, = np.nonzero(ytrue==ypred)
        #cnt2 = len(idx)
        bce = BinaryCrossEntropy(ytrue.reshape(-1,1),ypred.reshape(-1,1))
        cost = bce[0]
        print('tru shape, pred shape cost,skew t,skew p, kurt t, kurt p',ypred.shape[0],match_count,match_count-all_cld_count-all_clear_count,all_clear_count,all_cld_count,bce,skewt,skewp,kurtt,kurtp)
        #cost = np.abs(true_cnt-pred_cnt)
        #print('cost',cost,true_cnt,pred_cnt,bce)
        #cost = 1/cnt_perfect 
        print('whir',xx,np.abs(100*(np.abs(nomb)-np.abs(oomb))/np.abs(oomb))) 
        return np.abs(100*(np.abs(nomb)-np.abs(oomb))/np.abs(oomb))
    ...
    d_init = getParmAllSwMwOpen()
    replaceParms(result.x,d_init)
    writeNamelist('CRIS_CLDDET.NL_FINAL_BCE',d_init)
