...
def replaceParms(x,d):
    ...
    d['R__Window_Grad_Threshold'][1] = x[0]  
    d['N__GradChkInterval'][1] =int(x[1])  
...
if __name__ == "__main__":
    ...
    x00 = Real(name='x00',low=0.1, high=0.6)
    x01 = Integer(name='x01',low=4,high=6)
    ...
    def objective( x00, x01 ):
        ...
        omb = omb[:,consider_idx]
        omb_new = omb[np.where(ypred <1 )]
        omb_old = omb[np.where(ytrue <1 )]
        skew_old =  scipy.stats.skew(omb_old)
        skew_new =  scipy.stats.skew(omb_new)
        print('skew',skew_new,skew_old)
        #bce =  BCE()
        ...
        print('cost',cost,true_cnt,pred_cnt,np.abs(skew_new),np.abs(skew_old))
        #cost = 1/cnt_perfect 
        return np.abs(skew_new)
